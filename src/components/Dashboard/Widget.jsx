...
export default function DashboardWidget({ icon: Icon, bgColor, iconColor, title, value, statusCounter, handleClick }) {
    ...
    return (
        ...
        >
            <div className="w-full h-18 2xl:h-25 flex items-center justify-between">
                <div className={`h-16 w-16 2xl:h-20 2xl:w-20 rounded-full ${bgColor} flex items-center justify-center text-3xl`}>
                    <Icon className={iconColor} />
                </div>
            </div>
            <div className="w-full h-16 pl-2 mt-1">
                <div className="flex justify-between">
                    <div className="text-2xl 2xl:text-3xl font-bold text-lightDashboardWidgetComponentCountTextColor dark:text-darkWidgetText">{count}</div>
