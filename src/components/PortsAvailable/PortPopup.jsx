...
export default function PortPopup({ ip, handlePopupClose }) {
    ...
    return (
        <>
            {/* Heading */}
            <div className="min-h-12 flex items-center text-lg text-lightHeaderText justify-between w-full">
                <h1>IP : {ip}</h1>
                <CgCloseO className="text-xl" onClick={() => { handlePopupClose() }} />
            </div>

            <div className="h-12 w-full flex gap-5 items-center">
                <div className="flex gap-5 flex-grow">
                    <input className="flex h-12 w-full pl-3 rounded-md bg-lightInputElementBgColor dark:bg-darkInputElementBgColor text-lightInputElementTextColor" placeholder="Starting Range" name={"starting_range"} value={startingRange} onChange={(e)=>{setStartingRange(e.target.value)}}/>
                    <input className="flex h-12 w-full pl-3 rounded-md bg-lightInputElementBgColor dark:bg-darkInputElementBgColor text-lightInputElementTextColor" placeholder="Ending Range" name={"ending_range"} value={endingRange} onChange={(e) => { setEndingRange(e.target.value) }} />
                </div>
                <div>
                    <button className="h-12 w-30 bg-lightButton rounded-sm text-white max-h-[34px] flex justify-center items-center" onClick={handleFetch}>Fetch Data</button>
                </div>
            </div>
